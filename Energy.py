...
class EnergyPlotter:
    ...
    def total_energy_consumption(self,from_ts, to_ts):
        ...
        frame_layout = self.ui.Energy_kwh_frame.layout()
    ...
    def centerwise_piechart(self,from_ts, to_ts):
        ...
        frame_layout = self.ui.Energy_pie_frame.layout()
        if frame_layout is not None:
            for i in reversed(range(frame_layout.count())):
                frame_layout.itemAt(i).widget().setParent(None)
        else:
            frame_layout = QVBoxLayout(self.ui.Energy_pie_frame)
        frame_layout.addWidget(plot1)
    ...
    def weekly_graph(self,from_ts, to_ts):
        ...
        frame_layout = self.ui.Energy_weekplot_frame.layout()
        if frame_layout is not None:
            for i in reversed(range(frame_layout.count())):
                frame_layout.itemAt(i).widget().setParent(None)
        else:
            frame_layout = QVBoxLayout(self.ui.Energy_weekplot_frame)
        frame_layout.addWidget(plot1)   