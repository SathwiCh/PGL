...
#total energy consumption of energy center- 2 
    def ec02_energy_consumption(self,from_ts, to_ts):
        ...
        grouped = df_ec02.groupby(['building','Time column 1']).sum()
    ...
    def ec02_meter_piechart(self,from_ts,to_ts):
        ...
        frame_layout = self.ui.ec02_piechart_frame.layout()
        if frame_layout is not None:
            for i in reversed(range(frame_layout.count())):
                frame_layout.itemAt(i).widget().setParent(None)
        else:
            frame_layout = QVBoxLayout(self.ui.ec02_piechart_frame)
        frame_layout.addWidget(plot1)
    ...
    def ec02_fre_deviations(self,from_ts,to_ts):
        ...
        frame_layout = self.ui.ec02_Fdev_frame.layout()
        if frame_layout is not None:
            for i in reversed(range(frame_layout.count())):
                frame_layout.itemAt(i).widget().setParent(None)
        else:
            frame_layout = QVBoxLayout(self.ui.ec02_Fdev_frame)
        frame_layout.addWidget(plot1)
    ...
    def ec02_PF_deviations(self,from_ts,to_ts):
        ...
        frame_layout = self.ui.ec02_PFdev_frame.layout()
        if frame_layout is not None:
            for i in reversed(range(frame_layout.count())):
                frame_layout.itemAt(i).widget().setParent(None)
        else:
            frame_layout = QVBoxLayout(self.ui.ec02_PFdev_frame)
        frame_layout.addWidget(plot1)
    ...
    def ec02_Vswell_Vsag(self,from_ts,to_ts):
        ...
        frame_layout = self.ui.ec02_Vswellsag_frame.layout()
        if frame_layout is not None:
            for i in reversed(range(frame_layout.count())):
                frame_layout.itemAt(i).widget().setParent(None)
        else:
            frame_layout = QVBoxLayout(self.ui.ec02_Vswellsag_frame)
        frame_layout.addWidget(plot1)